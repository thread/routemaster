...
def _resync_states_on_state_machine(conn, machine):
    ...
    if deleted_states:
        _deprecate_states(conn, machine, deleted_states)

    if created_states:
        _create_or_undeprecate_states(conn, created_states, machine)

    any_changes = deleted_states or created_states
    return any_changes


def _create_or_undeprecate_states(conn, created_states, machine):
...
def _deprecate_states(conn, machine, deleted_states):
    conn.execute(
        states.update().where(
            and_(
                states.c.state_machine == machine.name,
