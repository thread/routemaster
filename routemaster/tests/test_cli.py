...
def test_cli_with_trivial_config(app_env):
    result = CliRunner(env=app_env).invoke(main, ['-c', 'test_data/trivial.yaml', 'validate'])
    assert result.exit_code == 0, result.output


def test_cli_with_invalid_config_fails(app_env):
    result = CliRunner(env=app_env).invoke(main, ['-c', 'test_data/disconnected.yaml', 'validate'])
